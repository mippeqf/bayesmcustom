...
output: 
  rmarkdown::html_document :
    ...
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{bayesmcustom Overview}
...
`bayesmcustom` is an R package that facilitates statistical analysis using Bayesian methods. The package provides a set of functions for commonly used models in applied microeconomics and quantitative marketing. 

The goal of this vignette is to make it easier for users to adopt `bayesmcustom` by providing a comprehensive overview of the package's contents and detailed examples of certain functions. We begin with the overview, followed by a discussion of how to work with `bayesmcustom`. The discussion covers the structure of function arguments, the required input data formats, and the various output formats. The final section provides detailed examples to demonstrate Bayesian inference with the linear normal, multinomial logit, and hierarchical multinomial logit regression models.
...
For ease of exposition, we have grouped the package contents into:
 ...
 - Utility functions
 - S3 methods^[For example, calling the generic function `summary(x)` on an object `x` with class `bayesmcustom.mat` actually dispatches the method `summary.bayesmcustom.mat` on `x`, which is equivalent to calling `summary.bayes.mat(x)`. For a nice introduction, see _Advanced R_ by Hadley Wickham, available [online](http://adv-r.had.co.nz/OO-essentials.html).]
 - Datasets
...
*`bayesmcustom` offers the utility function `breg` with a related but limited set of capabilities as `runireg` --- similarly with `rmultireg` for `rsurGibbs`, `rmixGibbs` for `rnmixGibbs`, and `rhierBinLogit` for `rhierMnlRwMixture`. 
...
-------------------- ----------------------
`plot.bayesmcustom.mat`    `summary.bayesmcustom.mat`  

`plot.bayesmcustom.nmix`   `summary.bayesmcustom.nmix` 

`plot.bayesmcustom.hcoef`  `summary.bayesmcustom.var`  
-------------------------------------------
...
`bayesmcustom`'s posterior sampling functions do not accept data stored in dataframes; data must be stored as vectors or matrices.
...
MCMC methods are non-iid. As a result, a large simulation size may be required to get reliable results. We recommend setting `R` large enough to yield an adequate effective sample size and letting `keep` default to the value 1 unless doing so imposes memory constraints. A careful reader of the `bayesmcustom` help files will notice that many of the examples set `R` equal to 1000 or less. This low number of draws was chosen for speed, as the examples are meant to demonstrate _how_ to run the code and do not necessarily suggest best practices for a proper statistical analysis.
...
`bayesmcustom` posterior sampling functions return a consistent set of results and output to the user. At a minimum, this output includes draws from the posterior distribution. `bayesmcustom` provides a set of `summary` and `plot` methods to facilitate analysis of this output, but the user is free to analyze the results as he or she sees fit.

### Output Formats

All `bayesmcustom` posterior sampling functions return a list. The elements of that list include a set of vectors, matrices, and/or arrays (and possibly a list), the exact set of which depend on the function.
...
In R generally, objects can be assigned a _class_ and then a _generic_ function can be used to run a _method_ on an object with that class. The list elements in the output from `bayesmcustom` posterior sampling functions are assigned special `bayesmcustom` classes. The `bayesmcustom` package includes `summary` and `plot` methods for use with these classes (see the table in Section 2 above). This means you can call the generic function (e.g., `summary`) on individual list elements of `bayesmcustom` output and it will return specially-formatted summary results, including the effective sample size. 

To see this, the code below provides an example using `runireg`. Here the generic function `summary` dispatches the method `summary.bayesmcustom.mat` because the `betadraw` element of `runireg`'s output has class `bayesmcustom.mat`. This example also shows the information about the prior that is printed to the console during the call to a posterior sampling function. Notice, however, that no remaining time is printed because `nprint` is set to zero.
...
`bayesmcustom` was originally created as a companion to BSM, at which time most functions were written in R. The package has since been expanded to include additional functionality and most code has been converted to C++ via `Rcpp` for faster performance. However, for users interested in obtaining the original implementation of a posterior sampling function (in R instead of C++), you may still access the last version (2.2-5) of `bayesmcustom` prior to the C++/`Rcpp` conversion from the [package archive](https://cran.r-project.org/src/contrib/Archive/bayesmcustom/) on CRAN. 

To access the `R` code in the current version of `bayesmcustom`, the user can simply call a function without parenthesis. For example, `bayesmcustom::runireg`. However, most posterior sampling functions only perform basic checks in `R` and then call an unexported C++ function to do the heavy lifting (i.e., the MCMC draws). This C++ source code is not available to the user via the installed `bayesmcustom` package because C++ code is compiled upon package installation on Linux machines and pre-compiled by CRAN for Mac and Windows. To access this source code, the user must download the "package source" from CRAN. This can be accomplished by clicking on the appropriate link at the `bayesmcustom` [package archive](https://cran.r-project.org/src/contrib/Archive/bayesmcustom/) or by executing the `R` command `download.packages(pkgs="bayesmcustom", destdir=".", type="source")`.  Either of these methods will provide you with a compressed file "bayesmcustom_version.tar.gz" that can be uncompressed. The C++ code can then be found in the "src" subdirectory.
...
`bayesmcustom`'s posterior sampling functions (as their name suggests) sample from posterior distributions. `bayesmcustom`'s `summary` and `plot` methods can be used to analyze those draws. Unlike most classical econometric methods, the MCMC methods implemented in `bayesmcustom`'s posterior sampling functions provide an estimate of the entire posterior distribution, not just a few moments. Given this "rich" result from Bayesian methods, it is best to summarize posterior distributions using histograms or quantiles. We advise that you resist the temptation to simply report the posterior mean and standard deviation; for non-normal distributions, those moments may have little meaning.
...
Conveniently, `bayesmcustom` offers this functionality (and more) with its `summary` and `plot` methods. Notice that `bayesmcustom`'s methods use a default burn-in length, calculated as `trunc(0.1*nrow(X))`. You can override the default by specifying the `burnin` argument. We see that the means for the first few retail fixed effects in the summary information below match those calculated "by hand" above.

```{r}
summary(out$betadraw)
```

The same can be done with the `plot` generic function. However, we reference the method directly (`plot.bayesmcustom.mat`) to plot a subset of the `bayesmcustom` output -- specifically we plot the price coefficient. In the histogram, note that the green bars delimit a 95\% Bayesian credibility interval, yellow bars shows +/- 2 numerical standard errors for the posterior mean, and the red bar indicates the posterior mean. Also notice that this pink histogram of the posterior distribution on price, which was created by calling the `plot` generic, matches the blue one we created "by hand" above.
...
Compared to the standard $n \times k$ rectangular format for data to be used in a linear regression model, choice data may be stored in various formats, including a rectangular format where the $p$ choice alternatives are allocated across columns or rows, or a list-of-lists format as used in `bayesmcustom`'s hierarchical models, which we demonstrate in Example 3 below. For all functions --- and notably those that implement multinomial logit and probit models --- the data must be in the format expected by the function, and `bayesmcustom`'s posterior sampling funtions are no exception. 
...
We use `bayesmcustom`'s default values for the parameters of the priors: $\bar{\beta} = 0$ and $A = 0.01I$.
...
The `camera` data is stored in a list-of-lists format, which is the format required by `bayesmcustom`'s posterior sampling functions for hierarchical models. This format has one list per individual with each list containing a vector `y` of choice outcomes and a matrix `X` of covariates. As with the multinomial logit model of the last example, `y` is a length-$n_i$ vector (or one-column matrix) and `X` has dimensions $n_ij \times k$ where $n_i$ is the number of choice occasions faced by individual $i$, $j$ is the number of choice alternatives, and $k$ is the number of covariates. For the `camera` data, $N=332$, $n_i=16$ for all $i$, $j=5$, and $k=10$.
...
We hope that this vignette has provided the reader with an introduction to the `bayesmcustom` package and is sufficient to enable immediate use of its posterior sampling functions for bayesian estimation.
