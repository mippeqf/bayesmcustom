...
vec condmom(vec const &x, vec const &mu, mat const &sigmai, int p, int j)
{
  ...
  for (int i = 0; i < p; i++)
    if (i != jm1)
      m += -csigsq * sigmai(ind + i) * (x[i] - mu[i]);
...
vec q0(mat const &y, lambda const &lambda_struct)
{
  ...
  if (k > 1)
  {
    vec km1(k - 1);
    for (int i = 0; i < (k - 1); i++)
      km1[i] = i + 1; // vector of 1:k SEE SEQ_ALONG
...
vec seq_rcpp(double from, double to, int len)
{
  ...
  vec res(len);
  res[len - 1] = to;
  res[0] = from; // note the order of these two statements is important, when gridsize = 1 res[0] will be rewritten to the correct number
