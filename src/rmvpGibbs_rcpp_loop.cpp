...
void draww_mvp(vec &outw, vec const &w, vec const &mu, mat const &sigmai, ivec const &y, int const t, int const &p, int const &nperthread)
{
...
vec init_threads(vec const &w, vec const &mu, mat const &sigmai, ivec const &y)
{
  // Distribute work over {threadcount} threads with each working through {nperthread} observations.
  // Since the outw vector is directly passed by reference, the threads are working directly on the output.
  // Hence once all threads are done, outw contains the new results without any manual merging.
  int p = sigmai.n_cols;
  int n = w.size() / p;
  ...
  threadcount = 10;
  int nperthread = std::ceil(n / threadcount);
  std::vector<std::thread> threadPool;

  for (int t = 0; t < threadcount; ++t)
  {
    cout << "init thread" << t;
    threadPool.emplace_back(draww_mvp, std::ref(outw), std::cref(w), std::cref(mu), std::cref(sigmai), std::cref(y), std::cref(t), std::cref(p), std::cref(nperthread));
  }
                         ...
                         mat const &V, double nu, vec const &betabar, mat const &A)
{
  ...
  for (int rep = 0; rep < R; rep++)
  {
    ...
    wnew = init_threads(wold, X * betaold, sigmai, y);
    cout << wnew.subvec(0, 5) << "\n --------------- \n n: " << wnew.size() / p << " p: " << p << "\n\n\n";
