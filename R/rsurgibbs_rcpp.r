...
#
if(missing(Prior))
   { betabar=c(rep(0,nvar)); A=bayesmcustomConstant.A*diag(nvar); nu=nreg+bayesmcustomConstant.nuInc; V=nu*diag(nreg)}
else
   {
    ...
    if(is.null(Prior$nu)) {nu=nreg+bayesmcustomConstant.nuInc}
       else {nu=Prior$nu}
