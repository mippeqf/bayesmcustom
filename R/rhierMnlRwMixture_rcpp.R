...
if(is.null(Prior$Amu) & sum(abs(SignRes))==0) {
  Amu=matrix(bayesmcustomConstant.A,ncol=1)
  } else {
  if(is.null(Prior$Amu) & sum(abs(SignRes)) >0) {
  Amu=matrix(bayesmcustomConstant.A*10,ncol=1)
...
if(is.null(Prior$nu) & sum(abs(SignRes))==0) {
  nu=nvar+bayesmcustomConstant.nuInc
  } else {
  if(is.null(Prior$nu) & sum(abs(SignRes)) >0) {
  nu=nvar+bayesmcustomConstant.nuInc+12
  } else {
  nu=Prior$nu } }
...
if(is.null(Prior$a)) { a=rep(bayesmcustomConstant.a,ncomp)} else {a=Prior$a}
...
attributes(draws$nmix)$class="bayesmcustom.nmix"
