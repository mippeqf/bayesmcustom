...
#
if(missing(Prior)) {
    betabar=rep(0,nvar); A=bayesmcustomConstant.A*diag(nvar) ;  a=bayesmcustomConstant.agammaprior; b=bayesmcustomConstant.bgammaprior;
}
else {
    if(is.null(Prior$betabar)) {betabar=rep(0,nvar)} else {betabar=Prior$betabar}
    if(is.null(Prior$A)) {A=bayesmcustomConstant.A*diag(nvar)} else {A=Prior$A}
...
if(is.null(Mcmc$s_alpha)) {cat("Using default s_alpha = 2.93",fill=TRUE); s_alpha=bayesmcustomConstant.RRScaling} 
    else {s_alpha = Mcmc$s_alpha} 
if(is.null(Mcmc$s_beta)) {cat("Using default s_beta = 2.93/sqrt(nvar)",fill=TRUE); s_beta=bayesmcustomConstant.RRScaling/sqrt(nvar)} 
...
attributes(draws$betadraw)$class=c("bayesmcustom.mat","mcmc")
