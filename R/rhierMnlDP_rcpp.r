...
# check on prior
#
alimdef=bayesmcustomConstant.DPalimdef
nulimdef=bayesmcustomConstant.DPnulimdef
vlimdef=bayesmcustomConstant.DPvlimdef
...
if(is.null(Prior$Prioralpha)) {Prioralpha=list(Istarmin=bayesmcustomConstant.DPIstarmin,Istarmax=min(50,0.1*nlgt),power=bayesmcustomConstant.DPpower)}
   else {Prioralpha=Prior$Prioralpha;
       if(is.null(Prioralpha$Istarmin)) {Prioralpha$Istarmin=bayesmcustomConstant.DPIstarmin} else {Prioralpha$Istarmin=Prioralpha$Istarmin}
       if(is.null(Prioralpha$Istarmax)) 
       {Prioralpha$Istarmax=min(50,0.1*nlgt)} else {Prioralpha$Istarmax=Prioralpha$Istarmax}
      if(is.null(Prioralpha$power)) {Prioralpha$power=bayesmcustomConstant.DPpower}
   }	
gamma= bayesmcustomConstant.gamma
...
if(is.null(Prior$Ad) & drawdelta) {Ad=bayesmcustomConstant.A*diag(nvar*nz)} else {Ad=Prior$Ad}
...
if(missing(Mcmc)) 
  {pandterm("Requires Mcmc list argument")}
   ...
   { 
    if(is.null(Mcmc$s)) {s=bayesmcustomConstant.RRScaling/sqrt(nvar)} else {s=Mcmc$s}
    if(is.null(Mcmc$w)) {w=bayesmcustomConstant.w}  else {w=Mcmc$w}
    if(is.null(Mcmc$keep)) {keep=bayesmcustomConstant.keep} else {keep=Mcmc$keep}
    if(is.null(Mcmc$nprint)) {nprint=bayesmcustomConstant.nprint} else {nprint=Mcmc$nprint}
      if(nprint<0) {pandterm('nprint must be an integer greater than or equal to 0')} 
    if(is.null(Mcmc$maxuniq)) {maxuniq=bayesmcustomConstant.DPmaxuniq} else {keep=Mcmc$maxuniq}
    if(is.null(Mcmc$gridsize)) {gridsize=bayesmcustomConstant.DPgridsize} else {gridsize=Mcmc$gridsize}
...
out = rhierMnlDP_rcpp_loop(R,keep,nprint,
                           ...
                           bayesmcustomConstant.A,bayesmcustomConstant.nuInc,bayesmcustomConstant.DPalpha)
###################################################################

if(drawdelta){
  attributes(out$Deltadraw)$class=c("bayesmcustom.mat","mcmc")
  attributes(out$Deltadraw)$mcpar=c(1,R,keep)}
attributes(out$betadraw)$class=c("bayesmcustom.hcoef")
attributes(out$nmix)$class="bayesmcustom.nmix"
attributes(out$adraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$nudraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$vdraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$Istardraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$alphadraw)$class=c("bayesmcustom.mat","mcmc")
