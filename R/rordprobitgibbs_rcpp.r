...
#
if(missing(Prior))
   { betabar=c(rep(0,nvar)); A=bayesmcustomConstant.A*diag(nvar); Ad=diag(ndstar); dstarbar=c(rep(0,ndstar))}
else
   {
    if(is.null(Prior$betabar)) {betabar=c(rep(0,nvar))} 
       else {betabar=Prior$betabar}
    if(is.null(Prior$A)) {A=bayesmcustomConstant.A*diag(nvar)} 
       else {A=Prior$A}
...
if(missing(Mcmc)) {pandterm("requires Mcmc argument")}
else
   {
    ...
    if(is.null(Mcmc$s)) {s=bayesmcustomConstant.RRScaling/sqrt(ndstar)} else {s=Mcmc$s} 
    }
#
# print out problem
#
...
attributes(draws$cutdraw)$class="bayesmcustom.mat"
attributes(draws$betadraw)$class="bayesmcustom.mat"
attributes(draws$dstardraw)$class="bayesmcustom.mat"
