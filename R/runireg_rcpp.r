...
#
if(missing(Prior))
   { betabar=c(rep(0,nvar)); A=bayesmcustomConstant.A*diag(nvar); nu=bayesmcustomConstant.nu; ssq=var(y)}
else
   {
    ...
    if(is.null(Prior$nu)) {nu=bayesmcustomConstant.nu}
       else {nu=Prior$nu}
...
attributes(draws$sigmasqdraw)$class=c("bayesmcustom.mat","mcmc")
