...
nu = p+bayesmcustomConstant.nuInc
V= nu*diag(p)
mubar = matrix(rep(k/2,p),ncol=1)
Am = bayesmcustomConstant.A*diag(p)
...
R = out$ndpost
###################################################################
attributes(out$drmu)$class=c("bayesmcustom.mat","mcmc")
attributes(out$drmu)$mcpar=c(1,R,keep)
attributes(out$drtau)$class=c("bayesmcustom.mat","mcmc")
attributes(out$drtau)$mcpar=c(1,R,keep)
attributes(out$drsigma)$class=c("bayesmcustom.mat","mcmc")
attributes(out$drsigma)$mcpar=c(1,R,keep)
attributes(out$drLambda)$class=c("bayesmcustom.mat","mcmc")
attributes(out$drLambda)$mcpar=c(1,R,keep)
attributes(out$dre)$class=c("bayesmcustom.mat","mcmc")
attributes(out$dre)$mcpar=c(1,R,keep)
attributes(out$drSigma)$class=c("bayesmcustom.var","bayesmcustom.mat","mcmc")
attributes(out$drSigma)$mcpar=c(1,R,keep)
