...
if(missing(Prior)) {pandterm("requires Prior argument ")}
else
   {
    ...
    if(is.null(Prior$Prioralpha)) {Prioralpha=list(Istarmin=bayesmcustomConstant.DPIstarmin,Istarmax=min(50,0.1*nobs),power=bayesmcustomConstant.DPpower)}
...
if(missing(Mcmc)) {pandterm("requires Mcmc argument")}
else
   {
    ...
    if(is.null(Mcmc$maxuniq)) {maxuniq=bayesmcustomConstant.DPmaxuniq} else {maxuniq=Mcmc$maxuniq}
    if(is.null(Mcmc$SCALE)) {SCALE=bayesmcustomConstant.DPSCALE} else {SCALE=Mcmc$SCALE}
...
attributes(nmix)$class="bayesmcustom.nmix"
