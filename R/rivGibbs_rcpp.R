...
#
if(missing(Prior))
   { md=c(rep(0,dimd));Ad=bayesmcustomConstant.A*diag(dimd); 
     mbg=c(rep(0,(1+dimg))); Abg=bayesmcustomConstant.A*diag((1+dimg));
     nu=3; V=diag(2)}
else
   {
    if(is.null(Prior$md)) {md=c(rep(0,dimd))} 
       else {md=Prior$md}
    if(is.null(Prior$Ad)) {Ad=bayesmcustomConstant.A*diag(dimd)} 
       else {Ad=Prior$Ad}
    if(is.null(Prior$mbg)) {mbg=c(rep(0,(1+dimg)))} 
       else {mbg=Prior$mbg}
    if(is.null(Prior$Abg)) {Abg=bayesmcustomConstant.A*diag((1+dimg))} 
       else {Abg=Prior$Abg}
...
attributes(draws$gammadraw)$class=c("bayesmcustom.mat","mcmc")
attributes(draws$gammadraw)$mcpar=c(1,R,keep)
attributes(draws$Sigmadraw)$class=c("bayesmcustom.var","bayesmcustom.mat","mcmc")
