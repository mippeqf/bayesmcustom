...
else
{
  ...
  if(is.null(Prior$A)) {A=matrix(bayesmcustomConstant.A,ncol=1)} 
  else {A=Prior$A}
  if(is.null(Prior$nu)) {nu=dimy+bayesmcustomConstant.nuInc} 
  else {nu=Prior$nu}
  if(is.null(Prior$V)) {V=nu*diag(dimy)} 
  else {V=Prior$V}
  if(is.null(Prior$a)) {a=c(rep(bayesmcustomConstant.a,ncomp))}
  else {a=Prior$a}
}
#
