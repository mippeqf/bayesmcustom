...
if(missing(Prior)) {  
  ...
  A=bayesmcustomConstant.A*diag(K)
  theta_hat=rep(0,K)
  nu0=K+1
  s0_sq=1
  deltabar=rep(0,I)
  Ad=bayesmcustomConstant.A*diag(I)
  VOmega=bayesmcustomConstant.BLPVOmega
}
else {
  ...
  if(is.null(Prior$A)) {A=bayesmcustomConstant.A*diag(K)} else {A=Prior$A}
  ...
  if(is.null(Prior$Ad)) {Ad=bayesmcustomConstant.A*diag(I)} else {Ad=Prior$Ad}
  if(is.null(Prior$VOmega)) {VOmega=bayesmcustomConstant.BLPVOmega} else {VOmega=Prior$VOmega}
}
...
if(is.null(Mcmc$s)+is.null(Mcmc$cand_cov)==2){
  s=bayesmcustomConstant.RRScaling/sqrt(K*(K+1)/2)
  cand_cov=diag(c(rep(0.1,K),rep(1,K*(K-1)/2)))
  tuning_auto=TRUE
}

if(is.null(Mcmc$tol)) {tol=bayesmcustomConstant.BLPtol} else {tol=Mcmc$tol}
minaccep=0.3
maxaccep=0.5
...
attributes(out$tausqdraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$tausqdraw)$mcpar=c(1,R,keep)
attributes(out$thetabardraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$thetabardraw)$mcpar=c(1,R,keep)
attributes(out$rdraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$rdraw)$mcpar=c(1,R,keep)
attributes(out$Sigmadraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$Sigmadraw)$mcpar=c(1,R,keep)
attributes(out$Omegadraw)$class=c("bayesmcustom.mat","mcmc")
