...
#
if(missing(Prior)) 
  { betabar=rep(0,k) ; A=bayesmcustomConstant.A*diag(k) ; nu=pm1+3; V=nu*diag(pm1)}
else 
  {if(is.null(Prior$betabar)) {betabar=rep(0,k)} else {betabar=Prior$betabar}
   ...
   if(is.null(Prior$nu)) {nu=pm1+bayesmcustomConstant.nuInc} else {nu=Prior$nu}
...
attributes(loopout$betadraw)$class=c("bayesmcustom.mat","mcmc")
attributes(loopout$betadraw)$mcpar=c(1,R,keep)
attributes(loopout$sigmadraw)$class=c("bayesmcustom.var","bayesmcustom.mat","mcmc")
