...
#
if(missing(Prior))
   { Deltabar=matrix(rep(0,nz*nvar),ncol=nvar); A=bayesmcustomConstant.A*diag(nz);
     nu.e=bayesmcustomConstant.nu.e; ssq=sapply(regdata,getvar) ; nu=nvar+bayesmcustomConstant.nuInc ; V= nu*diag(nvar)}
else
   {
    if(is.null(Prior$Deltabar)) {Deltabar=matrix(rep(0,nz*nvar),ncol=nvar)} 
       else {Deltabar=Prior$Deltabar}
    if(is.null(Prior$A)) {A=bayesmcustomConstant.A*diag(nz)} 
       else {A=Prior$A}
    if(is.null(Prior$nu.e)) {nu.e=bayesmcustomConstant.nu.e} 
       else {nu.e=Prior$nu.e}
    if(is.null(Prior$ssq)) {ssq=sapply(regdata,getvar)} 
       else {ssq=Prior$ssq}
    if(is.null(Prior$nu)) {nu=nvar+bayesmcustomConstant.nuInc} 
       else {nu=Prior$nu}
