...
if(missing(Prior))
   ...
   { 
    ...
    if(is.null(Prior$Ad)) Ad=diag(bayesmcustomConstant.A,dimd) else Ad=Prior$Ad
    if(isgamma) dimbg=1+dimg else dimbg=1
    if(is.null(Prior$mbg)) mbg=c(rep(0,dimbg)) else mbg=Prior$mbg
    if(is.null(Prior$Abg)) Abg=diag(bayesmcustomConstant.A,dimbg) else Abg=Prior$Abg
...
out = rivDP_rcpp_loop(R,keep,nprint,dimd,mbg,Abg,md,Ad,y,isgamma,z,x,w, 
                      ...
                      bayesmcustomConstant.A,bayesmcustomConstant.nu)
###################################################################
...
# methods (plot.bayesmcustom.nmix)
#
attributes(nmix)$class=c("bayesmcustom.nmix")

attributes(out$deltadraw)$class=c("bayesmcustom.mat","mcmc")
attributes(out$deltadraw)$mcpar=c(1,R,keep)
attributes(out$betadraw)$class=c("bayesmcustom.mat","mcmc")
...
if(isgamma){
  attributes(out$gammadraw)$class=c("bayesmcustom.mat","mcmc")
