...
#
if(missing(Prior)) {
    Deltabar=matrix(rep(0,nvar*nz),nrow=nz) ; Adelta=bayesmcustomConstant.A*diag(nz) ; nu=nvar+bayesmcustomConstant.nuInc; V=nu*diag(nvar); a=0.5; b=0.1;
}
else {
    if(is.null(Prior$Deltabar)) {Deltabar=matrix(rep(0,nvar*nz),nrow=nz)} else {Deltabar=Prior$Deltabar}
    if(is.null(Prior$Adelta)) {Adelta=bayesmcustomConstant.A*diag(nz)} else {Adelta=Prior$Adelta}
    if(is.null(Prior$nu)) {nu=nvar+bayesmcustomConstant.nuInc} else {nu=Prior$nu}
    if(is.null(Prior$V)) {V=nu*diag(nvar)} else {V=Prior$V}
    if(is.null(Prior$a)) {a=bayesmcustomConstant.agammaprior} else {a=Prior$a}
    if(is.null(Prior$b)) {b=bayesmcustomConstant.bgammaprior} else {b=Prior$b}
}
...
if(is.null(Mcmc$s_alpha)) { s_alpha=bayesmcustomConstant.RRScaling} 
    else {s_alpha= Mcmc$s_alpha }
if(is.null(Mcmc$s_beta)) { s_beta=bayesmcustomConstant.RRScaling/sqrt(nvar)} 
    else {s_beta=Mcmc$s_beta }
if(is.null(Mcmc$w)) { w=bayesmcustomConstant.w} 
    else {w = Mcmc$w}
...
attributes(draws$alphadraw)$class=c("bayesmcustom.mat","mcmc")
attributes(draws$alphadraw)$mcpar=c(1,R,keep)
attributes(draws$Deltadraw)$class=c("bayesmcustom.mat","mcmc")
attributes(draws$Deltadraw)$mcpar=c(1,R,keep)
attributes(draws$Vbetadraw)$class=c("bayesmcustom.var","bayesmcustom.mat","mcmc")
attributes(draws$Vbetadraw)$mcpar=c(1,R,keep)
attributes(draws$Betadraw)$class=c("bayesmcustom.hcoef")
